...
const char* DEFAULT_SSID = "Mr.Gk";
const char* DEFAULT_PASSWORD = "Mr.Gk1992"; 
...
String hashPassword(const String& password) {
  SHA256 sha;
  sha.update(password.c_str(), password.length());
...
<head>
  ...
  <style>
    ...
    .io-control { margin-top: 10px; }
    .tab { display: none; }
    .active { display: block; }
    nav { background: #007bff; padding: 10px; margin-bottom: 20px; border-radius: 5px; }
    nav a { color: white; text-decoration: none; margin-right: 15px; }
    table { width: 100%; border-collapse: collapse; }
    th, td { border: 1px solid #ddd; padding: 8px; text-align: left; }
    th { background-color: #f2f2f2; }
    .form-group { margin-bottom: 10px; }
    button { background: #007bff; color: white; border: none; padding: 5px 10px; border-radius: 3px; cursor: pointer; }
...
<body>
  <nav>
    ...
    <a href="#" onclick="showTab('io')">I/O Control</a>
    <a href="#" onclick="showTab('scripts')">Scripts</a>
  ...
  <div id="dashboard" class="tab active">
    ...
    <div class="card">
      <h3>Input Status</h3>
      <div class="status">
        <div>DI1: <span id="di1-status">?</span></div>
        <div>DI2: <span id="di2-status">?</span></div>
        <div>ADC: <span id="adc-value">?</span></div>
      </div>
    </div>
    <div class="card">
      <h3>Output Status</h3>
      <div class="status">
        <div>DO1: <span id="do1-status">?</span></div>
        <div>DO2: <span id="do2-status">?</span></div>
        <div>Script: <span id="script-type">None</span></div>
      </div>
    </div>
  </div>
  ...
  <div id="io" class="tab">
    ...
    <div class="card">
      ...
      <div class="form-group">
        <label>DO1:</label>
        <input type="checkbox" id="do1-switch" onchange="updateOutputs()">
      </div>
      <div class="form-group">
        <label>DO2 (PWM):</label>
        <input type="range" id="do2-pwm" min="0" max="255" oninput="document.getElementById('pwm-value').textContent=this.value">
        <span id="pwm-value">0</span>
      </div>
      <button onclick="updateOutputs()">Apply Changes</button>
    </div>
  </div>
  ...
  <div id="scripts" class="tab">
    ...
    <div class="card">
      <h3>Script Editor</h3>
      <div class="form-group">
        <select id="script-selector" onchange="loadScriptForEditing()">
          <option value="">Select a script...</option>
        </select>
      </div>
      <textarea id="script-editor" rows="10" style="width:100%"></textarea>
      <button onclick="saveScript()">Save</button>
      <button onclick="loadAndRunScript()">Load & Run</button>
    </div>
  </div>
  ...
  <div id="settings" class="tab">
    ...
    <div class="card">
      ...
      <div class="form-group">
        ...
        <input type="number" id="modbus-slave-id" min="1" max="247">
      </div>
      <div class="form-group">
  ...
  <script>
    ...
    function updateStatus(data) {
      ...
      document.getElementById('script-type').textContent = 
        data.scriptType === 1 ? 'Ladder' : 
        data.scriptType === 2 ? 'Mini-Python' : 
        data.scriptType === 3 ? 'Lua' : 'None';
    }
        ...
        .then(data => {
          ...
          data.scripts.forEach(script => {
            ...
            row.innerHTML = `
              <td>${script.name}</td>
              <td>
                <button onclick="loadAndRunScript('${script.path}')">Load</button>
                <button onclick="deleteScript('${script.path}')">Delete</button>
              </td>
            `;
